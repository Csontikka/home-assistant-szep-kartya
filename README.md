...
- **Safer handling of the card data.** An invalid `card_number` or `card_code` is reported without echoing the value into the log, long digit runs are masked in logged responses, and requests do not follow redirects.
...
If `card_number` or `card_code` is invalid, Home Assistant reports it without showing the value. Older versions of this component put the value into the log in that case, so check old logs before sharing them.
...
A failed query keeps the last balance, so automations that compare old and new states do not see a fake drop. The sensors only become unavailable after 48 hours without a successful query, or when the portal rejected the card before any successful query.
...
In every case the entities stay in place. The same message is also shown in the `last_error` attribute.
